...
ifneq "$(MAXIM_PATH)" ""
MAXIM_PATH := $(subst \,/,$(MAXIM_PATH))          # Sanitize MAXIM_PATH for backslashes
LIBS_DIR := $(abspath $(MAXIM_PATH)/Libraries)    # Locate some other useful paths...
...
AUTOSEARCH ?= 1
ifeq ($(AUTOSEARCH), 1)
SRCS += $(wildcard $(addsuffix /*.c, $(VPATH)))    # Auto-detect all C source files on VPATH
endif
...
#################################################################################################################################
# !!!!!!!!!!! UNCOMMENT THIS LINKERFILE FOR USE WITH BOOTLOADER IN FIBER !!!!!!!!!!
# LINKERFILE=$(TARGET_LC).ld
#################################################################################################################################
...
# ^ Must sanitize path for \ on Windows, since this may come from an environment variable.
...
# Include the rules for building for this target. All other makefiles should be included before this one.
include $(CMSIS_ROOT)/Device/Maxim/$(TARGET_UC)/Source/$(COMPILER)/$(TARGET_LC).mk

# Include the rules that integrate the SBTs.  SBTs are a special case that must be include after the core gcc rules to extend them.
ifeq ($(SBT), 1)
